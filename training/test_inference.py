...
def load_model(checkpoint_path):
    ...
    model = LinearReLUDropoutLinearNet(ckpt["input_dim"], ckpt["n_classes"]).to(device)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()

    scaler = None
    if ckpt.get("scale_embeddings", False):
        scaler_path = os.path.join(os.path.dirname(checkpoint_path), "scaler.joblib")
        scaler = joblib.load(scaler_path)
    
    st = SentenceTransformer(ckpt["sentence_transformer_name"])
    label_classes = ckpt.get("label_classes", [])

    return model, label_classes, st, scaler

def predict(model, st, text, scaler=None):
    ...
    if scaler is not None:
        ...
        print(f"Scaled Embedding: {embedding}")

    x = torch.from_numpy(embedding).float().unsqueeze(0).to(device)  # [1, dim]
    with torch.no_grad():
        logits = model(x)
...
if __name__ == "__main__":
    model, label_classes, st, scaler = load_model("models/emotion_classifier_v2.pt")
    print(label_classes)
    text = """
...
Fuck the police
"""
    prediction = predict(model, st, text, scaler)
    print(prediction)
